...
环境变量：
    ...
    XRAY_CONFIG      可选  xray 配置路径（冷启动回写），默认 xray.json
...
用法：
    ...
    python3 discover.py --write-xray-config PATH 生成完整 xray.json（含 stats/api + 启用用户）
...
def _vless_inbound(uuid, port, users):
    """vless-ws inbound（含 stats 所需的 tag + 每用户 email）"""
    clients = [OrderedDict([('id', uuid)])]   # 节点自身（无 email，不计流量）
    seen = {uuid}
    for u in users:
        if u['uuid'] in seen:
            continue
        seen.add(u['uuid'])
        clients.append(OrderedDict([('id', u['uuid']), ('email', u['userId'])]))
    return OrderedDict([
        ('listen', '127.0.0.1'),
        ('port', port),
        ('protocol', 'vless'),
...
def build_xray_config(uuid, port, users):
    """完整 xray 配置：vless-ws + StatsService/HandlerService（api inbound + policy + routing）。
    这样运行中的 xray 从一开始就支持 statsquery(读流量) 与 adu/rmu(免重启增删用户)。"""
    ...
    return OrderedDict([
        ...
        ('inbounds', [
            _vless_inbound(uuid, port, users),
            OrderedDict([
                ('tag', 'api'),
                ('listen', api_host),
                ('port', int(api_port)),
...
def write_xray_config(path, uuid, users):
    with open(path, 'w') as f:
        json.dump(build_xray_config(uuid, xray_port(), users), f, ensure_ascii=False, indent=2)
...
def sync_xray_users_via_api(uuid, users, config_path):
    """以运行实例 inbounduser 为真值与启用用户 diff，adu/rmu 动态增删（免重启）；并回写配置供冷启动。"""
    tag = XRAY_VLESS_TAG
    ...
    if to_add:
        ...
        pi = _vless_inbound(uuid, xray_port(), [])
        pi['settings']['clients'] = add_clients      # adu 会 build inbound 提取用户，故须可 build
        payload = {'inbounds': [pi]}
    ...
    # 回写配置（冷启动一致：crash 重启后仍有当前用户）
    try:
        write_xray_config(config_path, uuid, users)
    except Exception as e:
        print('WARN: 回写 %s 失败: %s' % (config_path, e), file=sys.stderr)
...
def main():
    ...
    if '--write-xray-config' in argv:
        ...
        if err:
            print('ERROR: %s' % err, file=sys.stderr)
            return 1
        users = fetch_users(users_url()) if gateway_base() else None
        if users is None:
            if os.path.exists(out_path):
                print('USERS_FETCH_FAILED keep existing %s' % out_path)
                return 0
            users = []   # 首次启动：先写仅含派生 uuid 的基础配置
        write_xray_config(out_path, uuid, users)
        print('VLESS_UUID=%s' % uuid)
    ...
    config_path = os.environ.get('XRAY_CONFIG', 'xray.json')
    ...
    if users is not None:
        sync_xray_users_via_api(uuid, users, config_path)
