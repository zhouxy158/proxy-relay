...
用法：
    ...
    python3 discover.py --fast                   rev 门控的快速用户/代理同步（每 ~10s 跑，秒级生效）
...
def _save_rev(path, rev):
    try:
        with open(path, 'w') as f:
            f.write(str(rev) + '\n')
    except Exception as e:
        print('WARN: 写 rev 文件失败: %s' % e, file=sys.stderr)
...
def main():
    ...
    if '--fast' in argv:
        ...
        if cur == _load_rev(rev_file):
            return 0                       # 未变，秒退（常态，不写日志、不碰 xray）
        users = fetch_users(users_url(), node_id)
        if users is None:
            return 0                       # 拉取失败：不落 rev，下次重试
        sync_xray_users_via_api(uuid, users)
        sync_xray_outbounds(users)
        _save_rev(rev_file, cur)           # 全部成功后才落 rev
        print('快速同步: rev -> %s（用户/代理已对账）' % cur)
        return 0
